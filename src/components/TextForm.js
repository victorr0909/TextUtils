    ...
    const [text, setText]=useState('');
        return (
            ...
            <div className="container"  style={{color : props.mode===`dark`?`white`:`#08303f`}}>
                <h1>{props.heading} </h1>
                <div className="mb-3">      
                    <textarea className="form-control" value= {text} onChange={handleOnChange} style={{backgroundColor : props.mode===`light`?`white`:`#257a9a` , color : props.mode===`dark`?`white`:`#08303f`}} id="myBox" rows="8"></textarea>
                </div>
                ...
                <button className="btn btn-primary mx-2 my-1" onClick={handleToClick}>Clear</button>
